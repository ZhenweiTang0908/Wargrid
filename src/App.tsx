  ...
  return (
    <section className={`status ${team}`}>
      ...
      <div className="status-copy">
        ...
        <div className="skill-line">{unit.skill === 'guose' ? '国色/流离 · 方片乐/转移杀' : unit.skill === 'liuli' ? '流离 · 弃牌转移杀' : unit.skill === 'luoshen' ? '洛神/倾国 · 黑判获牌/黑牌作闪' : unit.skill === 'qingguo' ? '倾国 · 黑色手牌可当闪' : unit.skill === 'keji' ? '克己 · 未出杀则跳过弃牌' : unit.skill === 'kurou' ? '苦肉 · 失去体力并摸两张' : unit.skill === 'tieqi' ? '马术/铁骑 · 距离-1/红判禁闪' : unit.skill === 'mashu' ? '马术 · 计算距离时始终-1' : skillCopy[unit.skill]}{lordSkill}</div>
      </div>
    </section>
  )
}
