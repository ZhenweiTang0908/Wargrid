...
interface GameStore extends GameState {
  ...
  activateJieyin: () => void
  chooseJieyinTarget: (team: Team) => void
  activateRende: () => void
  activateKurou: () => void
  activateGuose: () => void
  activateLijian: () => void
...
function rescueDyingPlayer(state: GameState, startingHp: number) {
  ...
  while (hp <= 0) {
    ...
    working = payRescueCard(working, helper.id, peach)
...
function damage(state: GameState, attackerId: Team, targetId: Team, amount: number, message: string, skipRescue = false, sourceCard?: Card): Partial<GameState> {
  ...
  while (!skipRescue && targetId !== 'player' && hp <= 0) {
    const selfAid = selfRescueCard(state, { ...rescuedUnits[targetId], hand })
    if (selfAid) {
      const paid = payRescueCard({ ...state, units: { ...rescuedUnits, [targetId]: { ...rescuedUnits[targetId], hp, hand } }, deck: rescuedDeck, discard }, targetId, selfAid)
      rescuedUnits = paid.units; rescuedDeck = paid.deck; discard = paid.discard; hand = paid.units[targetId].hand; hp = paid.units[targetId].hp + 1
    ...
    rescuedUnits = paid.units; rescuedDeck = paid.deck; discard = paid.discard
  ...
  let units = { ...rescuedUnits, [targetId]: { ...rescuedUnits[targetId], hand, hp, revealed: hp <= 0 ? true : target.revealed, animation: playerRescued ? 'heal' as const : 'hit' as const } }
...
function borrowedSwordVictim(state: GameState, actorId: Team, wielderId: Team, forcedVictimId?: Team): Unit | undefined {
  const wielder = state.units[wielderId]
  return forcedVictimId ? state.units[forcedVictimId] : targetsFor(state, wielderId)
...
export function borrowedSwordChoices(state: GameState, actorId: Team, wielderId: Team, forcedVictimId?: Team): Card[] {
  const wielder = state.units[wielderId], victim = borrowedSwordVictim(state, actorId, wielderId, forcedVictimId)
  if (!wielder.equipment.weapon || !victim || !canBorrowedSwordTarget(state, wielder, victim)) return []
  return slashResponses(wielder).filter(card => {
    const equipped = equippedCards(wielder).some(item => item.id === card.id)
    const afterPayment = equipped ? { ...wielder, equipment: Object.fromEntries(Object.entries(wielder.equipment).filter(([, item]) => item?.id !== card.id)) as Unit['equipment'] } : wielder
    return combatDistance(state, afterPayment, victim) <= effectiveAttackRange(state, afterPayment)
  })
}
...
export const useGameStore = create<GameStore>((set, get) => ({
  ...
  dispatch: action => {
    ...
    if (action.type === 'END_TURN' && state.phase === 'player') {
      ...
      if (state.turnStage !== 'discard' && excess > 0 && !keji) {
        ...
        set({ turnStage: 'discard', discardSelection: [], selectedCardId: null, selectedAsSlash: false, selectedAsDismantle: false, qingnangMode: false, jieyinMode: false, jieyinSelection: [], lijianMode: false, lijianTargets: [], spearMode: false, spearSelection: [], jijiangSource: null, zhihengMode: false, zhihengSelection: [], chainTargets: [], reachable: [], pathPreview: [], message, history: log(state, message) }); return
      }
  ...
  respond: cardId => {
    ...
    if (pending.effect === 'dying') {
      if (card) {
        ...
        const rescuedPlayer = { ...player, ...(pending.target === 'player' ? { hp: nextHp, animation: 'heal' as const } : {}) }
        ...
        base = { ...base, units, pendingResponse: stillDying ? { ...pending, requiredCount, prompt: `${target.name}仍需 ${requiredCount} 张${rescuePrompt}，请继续使用或放弃` } : null, message, history: log(base, message) }
      } else {
  ...
  chooseQilinMount: slot => {
    const state = get(), pending = state.pendingQilin, attacker = state.units.player
  ...
  selectCard: id => {
    const state = get(); if (state.phase !== 'player' || state.pendingResponse || state.pendingGreenDragon || state.pendingLiuli || state.pendingAxe || state.pendingIceSword || state.pendingHalberd || state.pendingQilin) return
    ...
    if (state.jieyinMode) {
      ...
      set({ jieyinSelection, message: jieyinSelection.length === 2 ? '【结姻】请选择一名受伤男性角色' : `【结姻】请选择两张手牌（${jieyinSelection.length}/2）` }); return
    }
  ...
  activateJieyin: () => {
    ...
    if (!Object.values(state.units).some(unit => unit.id !== 'player' && unit.hp > 0 && unit.hp < unit.maxHp && unit.gender === 'male')) { set({ message: '没有可发动【结姻】的受伤男性角色' }); return }
    set({ jieyinMode: !state.jieyinMode, jieyinSelection: [], selectedCardId: null, selectedAsSlash: false, selectedAsDismantle: false, selectedAsFanjian: false, selectedAsRende: false, selectedAsGuose: false, qingnangMode: false, message: state.jieyinMode ? '已取消结姻' : '【结姻】请选择两张手牌' })
  },
  chooseJieyinTarget: team => {
    const state = get(), player = state.units.player, target = state.units[team]
    if (state.phase !== 'player' || state.turnStage !== 'play' || !state.jieyinMode || player.skillUsed || !player.skills.includes('jieyin') || team === 'player' || target.hp <= 0 || target.hp >= target.maxHp || target.gender !== 'male' || state.jieyinSelection.length !== 2) return
    const selected = new Set(state.jieyinSelection), paid = player.hand.filter(card => selected.has(card.id))
    if (paid.length !== 2) return
    const message = `${player.name}发动【结姻】，${target.name}回复 1 点体力${player.hp < player.maxHp ? `，${player.name}也回复 1 点体力` : ''}`
    set({ units: { ...state.units, player: { ...player, hp: Math.min(player.maxHp, player.hp + 1), hand: player.hand.filter(card => !selected.has(card.id)), skillUsed: true, animation: 'heal' }, [target.id]: { ...target, hp: target.hp + 1, animation: 'heal' } }, discard: [...state.discard, ...paid], jieyinMode: false, jieyinSelection: [], message, history: log(state, message) })
  },
  activateRende: () => {
    const state = get(), player = state.units.player
  ...
  runAI: async () => {
    ...
    for (const kind of ['ironChain', 'fireAttack', 'indulgence', 'dismantle', 'snatch', 'borrowedSword', 'arrows', 'barbarians', 'duel', 'slash'] as const) {
      ...
      if (state.pendingResponse || state.pendingLiuli || state.pendingAxe || state.pendingIceSword || state.pendingHalberd || state.pendingQilin) return
    ...
    for (let extra = 0; extra < extraSlashBudget; extra++) {
      ...
      await wait(420)
      if (get().pendingResponse || get().pendingLiuli || get().pendingAxe || get().pendingIceSword || get().pendingHalberd || get().pendingQilin || get().winner) return
    }
