...
export const isEquipment = (kind: CardKind) => ['crossbow', 'qinggang', 'greenDragon', 'spear', 'axe', 'halberd', 'qilinBow', 'shield', 'bagua', 'redHare', 'dilu'].includes(kind)
...
export function createInitialState(deck = createDeck()): GameState {
  const state: GameState = {
    ...
    units: {
      player: { id: 'player', name: '关羽', title: '美髯公', team: 'player', identity: 'lord', faction: 'shu', revealed: true, position: { x: 4, y: 8 }, hp: 5, maxHp: 5, hand: deck.slice(0, 4), equipment: {}, judgement: [], skill: 'wusheng', movement: 3, attacksUsed: 0, wineUsed: false, drunk: false, chained: false, animation: 'idle' },
      north: { id: 'north', name: '赵云', title: '少年将军', team: 'north', identity: 'loyalist', faction: 'shu', revealed: false, position: { x: 4, y: 0 }, hp: 4, maxHp: 4, hand: deck.slice(4, 8), equipment: {}, judgement: [], skill: 'longdan', movement: 3, attacksUsed: 0, wineUsed: false, drunk: false, chained: false, animation: 'idle' },
      east: { id: 'east', name: '夏侯惇', title: '独眼的罗刹', team: 'east', identity: 'rebel', faction: 'wei', revealed: false, position: { x: 8, y: 4 }, hp: 4, maxHp: 4, hand: deck.slice(8, 12), equipment: {}, judgement: [], skill: 'ganglie', movement: 3, attacksUsed: 0, wineUsed: false, drunk: false, chained: false, animation: 'idle' },
      west: { id: 'west', name: '司马懿', title: '狼顾之鬼', team: 'west', identity: 'renegade', faction: 'wei', revealed: false, position: { x: 0, y: 4 }, hp: 4, maxHp: 4, hand: deck.slice(12, 16), equipment: {}, judgement: [], skill: 'feedback', movement: 3, attacksUsed: 0, wineUsed: false, drunk: false, chained: false, animation: 'idle' },
    },
    deck: deck.slice(16), discard: [], phase: 'player', turnStage: 'play', turn: 1,
    scores: { player: 0, north: 0, east: 0, west: 0 }, turnOrder: ['player', 'north', 'east', 'west'], currentUnit: 'player', generalSelected: false, selectedUnit: 'player', selectedCardId: null, selectedAsSlash: false, selectedAsDismantle: false, spearMode: false, spearSelection: [], jijiangSource: null, discardSelection: [],
