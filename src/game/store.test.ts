...
describe('standard card scenarios', () => {
  ...
  it('lets Guo Jia gain a judgement card and draw two cards after damage', () => {
    ...
    useGameStore.getState().respond(null)
    let damaged = useGameStore.getState()
    expect(damaged.pendingYiji?.cards).toEqual([legacyA, legacyB])
    expect(damaged.units.player.hand).toEqual([])
    useGameStore.getState().chooseYijiRecipient(legacyA.id, 'west')
    useGameStore.getState().chooseYijiRecipient(legacyB.id, 'player')
    damaged = useGameStore.getState()
    expect(damaged.pendingYiji).toBeNull()
    expect(damaged.units.west.hand).toEqual([legacyA])
    expect(damaged.units.player.hand).toEqual([legacyB])
    expect(damaged.message).toContain('遗计】分配完毕')
  })
  ...
  it('triggers Yiji once for each point of damage', () => {
    ...
    expect(state.pendingYiji?.cards).toEqual(legacy)
    for (const item of legacy) useGameStore.getState().chooseYijiRecipient(item.id, 'player')
    state = useGameStore.getState()
    expect(state.units.player.hand).toEqual(legacy)
    expect(state.message).toContain('遗计】分配完毕')
  })
  ...
  it('lets Halberd attack up to three targets with the final hand slash', () => {
    ...
    useGameStore.getState().dispatch({ type: 'PLAY_CARD', unit: 'player', cardId: slash.id, target: 'north' })
    let state = useGameStore.getState()
    expect(state.pendingHalberd).toEqual({ cardId: slash.id, targets: ['north'] })
    expect(state.units.player.hand).toContainEqual(slash)
    useGameStore.getState().selectHalberdTarget('east')
    useGameStore.getState().selectHalberdTarget('west')
    useGameStore.getState().confirmHalberd()
    state = useGameStore.getState()
  ...
  it('does not let Bagua judge with the Arrows card still resolving', () => {
    const arrows = card('arrows'), bagua = card('bagua')
    useGameStore.setState(state => ({ deck: [], discard: [], units: {
      ...state.units,
      player: { ...state.units.player, hand: [arrows] },
      north: { ...state.units.north, hand: [], equipment: { armor: bagua } },
      east: { ...state.units.east, hp: 0 },
      west: { ...state.units.west, hp: 0 },
    } }))
    useGameStore.getState().dispatch({ type: 'PLAY_CARD', unit: 'player', cardId: arrows.id, target: 'north' })
  ...
  it('does not let Ganglie judge with the Arrows card still resolving', () => {
    ...
    useGameStore.getState().dispatch({ type: 'PLAY_CARD', unit: 'player', cardId: arrows.id, target: 'east' })
    const state = useGameStore.getState()
    expect(state.units.east.hp).toBe(3)
    expect(state.units.player.hp).toBe(5)
    expect(state.deck).toEqual([])
